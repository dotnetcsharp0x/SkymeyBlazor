...
namespace SkymeyBlazor.Model.Services
{
    public class UserService : DelegatingHandler 
    {
        ...
        {
            var resp = await new HttpClient().GetFromJsonAsync<List<SU_001ListViewModel>>("https://46.22.247.253:5007/api/User/GetUsers?token=" + token);
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            var isToServer = request.RequestUri?.AbsoluteUri.StartsWith("") ?? false;
